    ...
    return (
      <div className="w-11/12 lg:w-10/12 mx-auto py-16">
        ...
        <div className="flex justify-center mt-6">
          <Link href="/rooms">
            <button className="bg-linear-to-r from-[#FFD700] via-[#FFC107] to-[#FFB300] text-black font-medium px-6 py-3 rounded-full flex items-center gap-2 hover:scale-105 transition">
              Browse Rooms
              <LuMoveRight />
            </button>
          </Link>
    ...
    <div className="w-11/12 mx-auto py-16">
      <div>
        <div className="mb-10">
          ...
          <p className="text-white/80">Manage Your Room Bookings And Reservation</p>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-10 ">
        {data.map((booking) => (
          <div
            key={booking._id}
            className="p-8 border border-[#335483] rounded-2xl text-white bg-[#061a3a] flex justify-between"
          >
            <div className="flex gap-12">
             <div>
               <Image className="rounded-2xl" src={booking?.roomImage} alt="room-image" width={250} height={250}></Image>
             </div>
            ...
            <p className="text-white/80 text-sm">{booking.roomFloor === "1"
                ? "1st Floor"
                : booking.roomFloor === "2"
                ? "2nd Floor"
                : booking.roomFloor === "3"
                ? "3rd Floor"
                : `${booking.roomFloor}th Floor`}</p>
            </div>
              </div>
              <div className="flex items-center gap-10">
                <div className="flex flex-col  gap-2">
                  <p className="text-white/80 text-sm inline-flex items-center gap-2"><MdDateRange className="text-[#FFC107]" size={20}/>
           {booking.bookingDate ? format(new Date(booking.bookingDate), "dd MMMM yyyy") : "No Date"}
            </p>
            <p className="text-base font-semibold"> Date</p>
                </div>
                <div className="flex flex-col  gap-2">
                  <p className="text-white/80 text-sm inline-flex items-center gap-2"><BsStopwatch className="text-[#FFC107]" size={20}/>
           {booking.startTime} - {booking.endTime}
            </p>
            <p className="text-base font-semibold"> Time</p>
                </div>
                <div className="flex flex-col  gap-2">
                  <p className="text-white/80 text-sm inline-flex items-center gap-2"><AiFillDollarCircle className="text-[#FFC107]" size={20}/>
           ${booking.totalPrice}
            </p>
